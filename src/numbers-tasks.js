...
function roundToPowerOfTen(num, pow) {
  ...
  return Math.round(Math.round(num / divider) * divider);
}
...
function toPrecision(number, precision) {
  ...
  // representing this number to the specified precision.
...
function isNumber(number) {
  // Статический Number.isFinite()метод определяет, является ли переданное значение конечным
  // числом — то есть он проверяет, что данное значение является числом
